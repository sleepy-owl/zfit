...
from ..core.dimension import combine_spaces, get_same_obs
from ..core.basemodel import BaseModel
from ..core.interfaces import ZfitModel, ZfitSpace
from ..core.limits import no_norm_range, supports, Space
...
from ..util.exception import (ExtendedPDFError, AlreadyExtendedPDFError, AxesNotUnambiguousError,
                              ...
                              ModelIncompatibleError, LimitsIncompatibleError, NormRangeNotSpecifiedError)
...
class BaseFunctor(FunctorMixin, BaseModel):
    ...
    def _get_model_obs(self):
        return tuple(model.obs for model in self.models)

    def _infer_obs_from_daughters(self):
        obs = set(self._get_model_obs())
        if len(obs) == 1:
            return obs.pop()
        else:
            return False
    ...
    def _model_same_obs(self):
        return get_same_obs(self._get_model_obs())
...
class SumModel(BaseFunctor):

    def __init__(self, models: List[ZfitModel], fracs: Optional[ztyping.ParamTypeInput] = None, normalized=True,
        ...
        if len(models) < 2:
            raise ValueError("Cannot build a sum of a single model")  # STOPPED HERE
